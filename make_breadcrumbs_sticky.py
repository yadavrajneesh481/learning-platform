...
def replace_breadcrumb_css(file_path):
    ...
    try:
        ...
        # Pattern to match the entire breadcrumb CSS block including media query
        pattern = r'/\* Breadcrumb Navigation Styles.*?\n\s+@media \(max-width: 768px\).*?\}\s+\}'
        
        if re.search(pattern, content, re.DOTALL):
            # Replace with sticky CSS
            content = re.sub(pattern, sticky_breadcrumb_css.strip(), content, flags=re.DOTALL)
...
print("🔧 Making breadcrumbs sticky across ALL pages...\n")
total_fixed = 0
...
print(f"{'='*60}")
print("\n🎨 New sticky breadcrumb features:")
print("  • position: sticky - stays at top when scrolling")
print("  • z-index: 100 - appears above other content")
print("  • Stronger gradient background (95% opacity)")
print("  • White text for better visibility")
print("  • Gold current page indicator")
print("  • Removed border-radius for full-width appearance")
